...
<!-- Compact Filters (Single Line Layout) - WITH ROUTE FILTER ADDED -->
<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6 border border-gray-100 dark:border-gray-700 overflow-x-auto">
  ...
  <div class="flex flex-wrap lg:flex-nowrap items-end gap-4 min-w-max">
    <!-- ROUTE FILTER - NEW -->
    <div>
      <label for="route-filter" class="block text-[11px] font-medium text-gray-700 dark:text-gray-300 mb-1">
        Route
      </label>
      <select id="route-filter" class="w-36 px-2 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-1 focus:ring-green-500 focus:border-green-500">
    ...
    <table class="w-full border-collapse text-xs">
      <thead>
        <tr class="bg-green-600 dark:bg-green-700 text-white">
          ...
          <th class="border border-gray-300 dark:border-gray-600 px-2 py-2 text-center font-semibold">Route</th>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', async function() {
      ...
      try {
        ...
        populateRouteFilter(sampleData);
      ...
      // Add route filter change listener to update farmer filter
      document.getElementById('route-filter').addEventListener('change', updateFarmerFilterByRoute);
    });
    ...
    function showErrorMessage(message) {
      ...
      tableBody.innerHTML = `
        <tr>
          <td colspan="9" class="text-center py-6 text-red-600 font-semibold bg-red-50 dark:bg-red-900 dark:text-red-200">
            ${message}
          </td>
        </tr>`;
    }
    ...
    function populateRouteFilter(data) {
      ...
      const routes = [...new Set(data.map(i => i.route).filter(r => r))].sort();
    ...
    // MODIFIED: Update farmer filter based on selected route
    function updateFarmerFilterByRoute() {
      ...
      // Filter farmers by route if a route is selected
      let farmersToShow = sampleData;
      if (selectedRoute) {
        farmersToShow = sampleData.filter(i => i.route === selectedRoute);
      }
      
      const farmers = [...new Set(farmersToShow.map(i => i.farmer_id))].sort();
      farmers.forEach(farmerId => {
        const farmerData = farmersToShow.find(i => i.farmer_id === farmerId);
        const option = document.createElement('option');
        option.value = farmerId;
        option.textContent = `${farmerId} - ${farmerData.farmer_name}`;
    ...
    function displayData(data, page) {
      ...
      pageData.forEach(item => {
        ...
        row.innerHTML = `
          ...
          <td class="border border-gray-300 dark:border-gray-600 px-4 py-3 text-center text-gray-900 dark:text-gray-100">${item.route || '-'}</td>
    ...
    // MODIFIED: Apply filters with route filtering
    function applyFilters() {
      ...
      setTimeout(() => {
        ...
        // Apply route filter
        if (route) filtered = filtered.filter(i => i.route === route);
    ...
    // NEW FUNCTION: Reset all filters
    function resetFilters() {
      document.getElementById('route-filter').value = '';
      document.getElementById('farmer-filter').value = '';
      document.getElementById('date-from').value = '';
      document.getElementById('date-to').value = '';
      document.getElementById('status-filter').value = '';
      document.getElementById('level-filter').value = '';
      
      // Reset farmer filter to show all farmers
      updateFarmerFilterByRoute();
      
      // Reapply without filters
      filteredData = analyzeSupplySchedule(sampleData);
      currentPage = 1;
      displayData(filteredData, currentPage);
      setupPagination(filteredData);
